...
@protocol TRLoopDelegate <NSObject>
@required
- (void)trLoopUpdate:(ZCLoop *)loop;
@optional
- (void)trLoop:(ZCLoop *)loop willPause:(BOOL)pause;
@end

typedef void (^UpdateBlock)(NSTimeInterval timeSinceLastUpdate);

// The TRLoop class provides part of the standard view controller functionality, it implements loop using CADisplayLink (TODO: add NSTimer for compatibility with old devices). 
...
// Initialize loop without block. User should use delegate adopting |TRLoopDelegate| protocol
- (id)initWithFrameInterval:(NSUInteger)frameInterval;

// The view controller’s delegate.
@property (unsafe_unretained, nonatomic) id<TRLoopDelegate> delegate;
